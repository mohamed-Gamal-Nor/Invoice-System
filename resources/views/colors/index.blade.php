...
@section('content')
    <div class="row page-titles mx-0">
        ...
        <div class="col-sm-6 p-md-0 justify-content-sm-end mt-2 mt-sm-0 d-flex">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active"><a href="javascript:void(0);">جدول المقاسات & الالوان & وحدات المقياس</a></li>
                <li class="breadcrumb-item active"><a href="{{url('/'.$page='colors')}}">قائمة الالوان</a></li>
            </ol>
        </div>
    </div>
    @if ($errors->any())
        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="card-header">
                    <h4 class="card-title">قائمة الالوان</h4>
                    <button  class="btn btn-primary" data-toggle="modal" data-target=".product-section"><i class="las la-plus"></i> أضافة لون</button>
                </div>
                <div class="card-body">
                        ...
                        <table id="example3" class="display" style="min-width: 845px">
                            ...
                            @foreach($colors as $color)
                                ...
                                <tr>
                                    ...
                                    <td>{{$color->name}}</td>
                                    <td> <span class="badge badge-xl badge-warning" style="background-color: {{$color->rgb}}">*</span></td>
                                    <td>
                                    ...
                                    <td>{{$color->created_at->todatestring() }}</td>
                                    <td>
                                        @if(empty($color->updated_at))
                                            <span class="text-primary">لم يتم التعديل ع هذا القسم</span>
                                        @else
                                            {{$color->updated_at->todatestring() }}
                                        @endif

                                    </td>
                                    <td>
                                        <button  class="btn btn-sm btn-primary" data-toggle="modal" data-target=".product-section-edit"  data-id="{{ $color->id }}" data-section_name="{{ $color->name }}" data-color="{{ $color->rgb }}" data-description="{{ $color->description}}" ><i class="la la-pencil"></i></button>
                                        <button  class="btn btn-sm btn-danger" data-toggle="modal" data-target=".SectionDelete" data-id="{{ $color->id }}"><i class="la la-trash-o"></i></button>
                                    </td>
                                </tr>
                            @endforeach
                            </tbody>
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">أضافة لون جديد</h5>
                ...
                <form action="{{route('colors.store')}}" method="post">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group">
                            ...
                            <input type="color" class="form-control" name="rgb" value="#ff0000" required>
                        </div>
                        <div class="form-group">
                            <label>ملاحظات</label>
    ...
    <div class="modal fade product-section-edit" tabindex="-1" role="dialog" aria-hidden="true" id="product-section-edit">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">تعديل لون</h5>
                    <button type="button" class="close" data-dismiss="modal"><span>&times;</span>
                    </button>
                </div>
                <form action="{{route('colors.update','test')}}" method='post'>
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group">
                            <label>درجة اللون</label>
                            <input type="color" class="form-control" name="rgb"  id="color" required>
                        </div>
                        <div class="form-group">
                            <label>ملاحظات</label>
    ...
    <div class="modal fade SectionDelete" id="SectionDelete">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form action="{{ route('colors.destroy','test') }}" method="post">
                    {{ method_field('Delete') }}
                    @csrf
                    <input type="hidden" name="id" id="DeleteId" value="">
                    <div class="modal-body">

                        <p>هل تريد حذف اللون نهائيا؟ </p>
                        <p class="text-danger">ملحوظة : اذا كان هذا اللون مرتبط بعلاقات اخري لن يتم حذفه. </p>
                    </div>
                    <div class="modal-footer">
